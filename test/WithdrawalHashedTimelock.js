...
contract('WithdrawalHashedTimelock', accounts => {
    const sender = accounts[1]
    const receiver = accounts[2]

    it('newContract() should create new contract and store correct details', async () => {
        const hashPair = newSecretHashPair()
        const htlc = await WithdrawalHashedTimelock.deployed()
        ...
        const contract = withdrawalHtlcArrayToObj(contractArr)
    ...
    it('withdraw() should fail after timelock expiry', async () => {
        const hashPair = newSecretHashPair()
        const htlc = await WithdrawalHashedTimelock.new()
        ...
        return new Promise((resolve, reject) =>
            setTimeout(async () => {
                ...
                try {
                    ...
                    assert.isTrue(err.message.includes('withdrawable: timelock time must be in the future'))
                    resolve()
                }
            }, 1000)
        )
